...
def create_emulators ( sun_angles, x_min, x_max, n_train=250, n_validate=1000 ):
    ...
    will be reported too. The output will be two dictionaries, indexed by
    sun angle, one for VIS and one for NIR.
    
    """
    
    from GaussianProcess import GaussianProcess, lhd
    from lhd import *
    ...
    x_validate = np.random.rand (  n_train, n_params  )*(x_max - x_min) + \
        x_min
    emu_vis = {}
    emu_nir = {}
